...
// Método auxiliar recursivo para imprimir el árbol AVL con formato personalizado

    private void printTree(NodeAVL node, int depth) {
        if (node == null) {
            return;
        }

        // Imprimir el nodo actual con la sangría correspondiente
        for (int i = 0; i < depth; i++) {
            System.out.print("  ");
        }
        System.out.println(node.data);

        // Recorrer el subárbol izquierdo con una sangría adicional
        printTree(node.left, depth + 1);

        // Recorrer el subárbol derecho con la misma sangría
        printTree(node.right, depth + 1);
    }

// Método público para imprimir el árbol AVL con formato personalizado
